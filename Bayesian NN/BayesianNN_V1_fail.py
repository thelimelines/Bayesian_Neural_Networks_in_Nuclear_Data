...
def predict_binding_energy():
    ...
    predictions = np.array([model(input_data, training=True) for _ in range(100)])
    ...
    # Scale the mean prediction back to real output (keV)
    mean_prediction = scaler_y.inverse_transform(np.array([[mean_prediction]]))[0][0]
    
    # Check if the input data was in the training set
    in_training_set = any(np.all(X_train == input_data, axis=1))
    
    # Retrieve the actual value from the dataset
    actual_value = df.loc[(df['Neutron Number'] == neutron_number) & (df['Proton Number'] == proton_number)]
    actual_value = actual_value['Binding Energy per Nucleon (keV)'].values[0] if not actual_value.empty else 'N/A'
    
    # Update the result label with the mean prediction and the uncertainty
    result_label.config(text=f"In Training Set: {in_training_set}\nActual Value: {actual_value}\nPredicted Value: {mean_prediction:.2f} ± {std_dev_prediction:.2f}")
...
model = Sequential([
    tfp.layers.DenseVariational(16, input_dim=2, activation='relu',
                                make_posterior_fn=posterior_mean_field,
                                make_prior_fn=prior_trainable),
    tfp.layers.DenseVariational(16, activation='relu',
                                make_posterior_fn=posterior_mean_field,
                                make_prior_fn=prior_trainable),
